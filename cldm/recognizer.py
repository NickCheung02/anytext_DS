...
def min_bounding_rect(img):
    ...
    thresh = thresh[..., 0:1] if len(thresh.shape) == 3 else thresh
    contours, hierarchy = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if len(contours) == 0:
        cv2.imwrite("/home/610-zzy/AnyText2-main-Real0909-Calli/test/bad_mask_debug.png", img)  # 保存看看
...
def adjust_image(box, img):
    ...
    result = F.grid_sample(img.unsqueeze(0), grid.to(img.dtype), align_corners=True)
