...
###修改1##############################################################
    def __init__(self, control_stage_config, control_key, glyph_key, position_key, only_mid_control, loss_alpha=0, loss_beta=0, with_step_weight=False, use_vae_upsample=False, use_text_cond=False, use_text_emb=False, latin_weight=1.0, embedding_manager_config=None, training_stage=2,*args, **kwargs):
###修改1结束##########################################################
...
####修改4##############################################################
        if self.training_stage == 1:
            ...
            return diffusion_model(x=x_noisy, timesteps=t, context=img_cond)
####修改4结束##########################################################      
        img_cond = cond['c_crossattn'][0][0]
